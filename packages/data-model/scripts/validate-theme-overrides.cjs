...
ajv.addKeyword({
  ...
  validate: function validateThemeOverride(schema, data) {
    ...
    // Validate the override value against the value type's validation rules
    if (valueType.validation) {
      const value = data.value.value;
      
      // Validate pattern if present
      if (valueType.validation.pattern) {
      ...
      if (typeof value === 'number') {
        if (valueType.validation.minimum !== undefined && value < valueType.validation.minimum) {
          validateThemeOverride.errors = [{
            keyword: 'validateThemeOverride',
        ...
        if (valueType.validation.maximum !== undefined && value > valueType.validation.maximum) {
          validateThemeOverride.errors = [{
            keyword: 'validateThemeOverride',
      ...
      if (valueType.validation.allowedUnits && typeof value === 'string') {
