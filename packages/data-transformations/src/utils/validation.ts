import type { ValidationResult, ValidationError, ValidationWarning } from '../types/common';
 ...
 */
export function validateTokenSystemBasic(tokenSystem: TokenSystem): ValidationResult {
  ...
  if (!tokenSystem.tokenCollections || tokenSystem.tokenCollections.length === 0) {
    errors.push(createValidationError(
      'tokenCollections',
      'At least one token collection is required',
      'NO_TOKEN_COLLECTIONS'
    ));
  }
  ...
  if (!tokenSystem.resolvedValueTypes || tokenSystem.resolvedValueTypes.length === 0) {
    errors.push(createValidationError(
      'resolvedValueTypes',
      'At least one resolved value type is required',
      'NO_RESOLVED_VALUE_TYPES'
    ));
  }
...
export function validateReferentialIntegrity(tokenSystem: TokenSystem): ValidationResult {
    ...
    if (collection.resolvedValueTypeIds) {
      collection.resolvedValueTypeIds.forEach(typeId => {
        if (!resolvedValueTypeIds.has(typeId)) {
          errors.push(createValidationError(
            `tokenCollections[${index}].resolvedValueTypeIds`,
            `Referenced resolved value type '${typeId}' does not exist`,
            'INVALID_RESOLVED_VALUE_TYPE_REFERENCE',
            { collectionId: collection.id, referencedTypeId: typeId }
          ));
        }
      });
    }
  ...
  tokenSystem.tokens?.forEach((token, index) => {
    ...
    if (token.resolvedValueTypeId && !resolvedValueTypeIds.has(token.resolvedValueTypeId)) {
      errors.push(createValidationError(
        `tokens[${index}].resolvedValueTypeId`,
        `Referenced resolved value type '${token.resolvedValueTypeId}' does not exist`,
        'INVALID_RESOLVED_VALUE_TYPE_REFERENCE',
        { tokenId: token.id, referencedTypeId: token.resolvedValueTypeId }
      ));
    }
    ...
    if (token.tokenCollectionId && !collectionIds.has(token.tokenCollectionId)) {
      errors.push(createValidationError(
        `tokens[${index}].tokenCollectionId`,
        `Referenced token collection '${token.tokenCollectionId}' does not exist`,
        'INVALID_COLLECTION_REFERENCE',
        { tokenId: token.id, referencedCollectionId: token.tokenCollectionId }
      ));
    }
    ...
    token.valuesByMode?.forEach((valueByMode, valueIndex) => {
      valueByMode.modeIds?.forEach(modeId => {
        if (!modeIds.has(modeId)) {
          errors.push(createValidationError(
            `tokens[${index}].valuesByMode[${valueIndex}].modeIds`,
            `Referenced mode '${modeId}' does not exist`,
            'INVALID_MODE_REFERENCE',
            { tokenId: token.id, referencedModeId: modeId }
          ));
        }
      });
    });
    ...
    token.taxonomies?.forEach((taxonomyRef, taxIndex) => {
      if (!taxonomyIds.has(taxonomyRef.taxonomyId)) {
        errors.push(createValidationError(
          `tokens[${index}].taxonomies[${taxIndex}].taxonomyId`,
          `Referenced taxonomy '${taxonomyRef.taxonomyId}' does not exist`,
          'INVALID_TAXONOMY_REFERENCE',
          { tokenId: token.id, referencedTaxonomyId: taxonomyRef.taxonomyId }
        ));
      }
    });
  });
 ...
 */
export function validateRequiredFields(tokenSystem: TokenSystem): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationWarning[] = [];
  ...
  if (tokenSystem.systemId && !/^[a-zA-Z0-9-_]+$/.test(tokenSystem.systemId)) {
    errors.push(createValidationError(
      'systemId',
      'System ID must match pattern: ^[a-zA-Z0-9-_]+$',
      'INVALID_SYSTEM_ID_PATTERN',
